...
- **离线预生成的收件人不同**：「设置 → 识别与回答 → 预生成回答」在检测到本机装有 `claude` / `codex` CLI 时会优先用它，简历要点经由**你自己的 Anthropic / OpenAI 账号**，不经过上面那几个服务、也不消耗额度。设置页在按下按钮**之前**写明实际引擎，可在同一页关闭改用已配置的云端服务。实时面试路径从不使用本机 CLI。
