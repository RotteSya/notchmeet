...
final class ScriptImporterTests: XCTestCase {
    ...
    # 自己紹介
    サトウと申します。□□市で二年働きました。本日はよろしくお願いいたします。
    ...
    面接メモ 6/10 むけ、ざっくり
    まず自己紹介って言われたら→サトウです、□□市の動画会社で2年働いて、今は△△大学の院で経営を学んでます、って感じで軽く。
    志望動機聞かれた場合：課題を形にする仕事がしたい、御社は中立の立場でお客様に一番いい形を組めるから、って言う
    強みは？→相手の困りごとを聞いて、条件を整理して動ける形にする力。展示会のインターンの例を出す
    ...
    func testSegmentsRebuildVerbatimFromLineRanges() {
        ...
        XCTAssertEqual(entries[0].answer, "サトウと申します。\n□□市で二年働きました。")
    }

    /// 文字列回答は原文の逐字部分列でなければ拒否（LLM の言い換えを機械的に遮断）。
    func testStringAnswersMustBeVerbatimSubstrings() {
        let text = "自己紹介→サトウです、□□市で2年働いてました、って軽く言う"
        let ok = #"{"items":[{"q":"自己紹介","a":"サトウです、□□市で2年働いてました"}]}"#
        let rewritten = #"{"items":[{"q":"自己紹介","a":"サトウと申します。□□市で二年間勤務しておりました。"}]}"#
    ...
    func testLeadInPrefixesAreStrippedFromSegmentAnswers() {
        XCTAssertEqual(ScriptImporter.stripLeadIn("自己紹介きかれたら→サトウです。□□市で二年働きました。"),
                       "サトウです。□□市で二年働きました。")
    ...
    func testNormalizeUsesValidLLMSegmentation() async {
        let seg = """
        {"items":[
          {"q":"自己紹介","a":"サトウです、□□市の動画会社で2年働いて、今は△△大学の院で経営を学んでます"},
          {"q":"志望動機","a":"課題を形にする仕事がしたい、御社は中立の立場でお客様に一番いい形を組めるから"},
          {"q":"強みは？","a":"相手の困りごとを聞いて、条件を整理して動ける形にする力。展示会のインターンの例を出す"},
        ...
        XCTAssertTrue(r.entries[0].answer.contains("□□市の動画会社で2年働いて"))
