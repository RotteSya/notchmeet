...
final class LLMRouter: Router {
    ...
    static func isDeictic(_ question: String) -> Bool {
        anaphoraMarkers.contains(where: question.contains)
    }

    /// 只认指向**直前回答**的明确回指。这个判定的下游是硬否决（deicticVeto）与
    /// grounding 查询收窄，弱标记的误伤是双杀：「前面说的/上面说的」可指简历、
    /// 共享屏幕上的文档、几轮之前的话题，所指滑出 history 窗口时，命中被否决、
    /// 正确原稿也被挤出上下文——所以不进表；「那么/那我们」是开场语，同样不进。
    private static let anaphoraMarkers = [
        "それを", "それが", "それって", "その経験", "その学び", "その強み",
        "その話", "先ほどの", "さっきの", "今の話",
        "刚才", "刚刚", "刚提到", "你说的那", "你提到的", "你刚说", "刚说的",
