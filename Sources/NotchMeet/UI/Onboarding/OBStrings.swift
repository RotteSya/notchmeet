...
struct OBStrings {
    ...
    // v3 — 目标步（面试语言 + 公司 + 岗位；每个字段当场说明它改变什么行为）。
    let kTarget, hTarget, pTarget: String
    let lblInterviewLanguage, capInterviewLanguage: String
    let phCompany, capCompany, phRole, capRole: String
    let sumTargetLabel: String
    // v3 — 装填弹药：导入步同时接受简历（PDF/MD/TXT），完成后交给工作台逐条确认。
    let btnPickAmmo, resumeRecogFormat, sumResumeLabel: String
    ...
    static let zh = OBStrings(
        ...
        k1: "STEP 2 · 装填弹药", h1step: "导入你准备好的回答",
        ...
        k2: "STEP 3 · 访问权限", h2step: "聆听面试官的声音",
        ...
        btnAllow: "允许访问",
        k3: "STEP 5 · 演示", h3step: "聆听日语，使用日语作答",
        ...
        kKeys: "STEP 4 · 连接服务", hKeys: "粘贴激活码即可使用",
        ...
        kGift: "STEP 4 · 见面礼", hGiftGranted: "60 分钟，已到账", hGiftBalance: "你的可用额度",
        pGift: "无需任何设置，装好即用。额度只在面试录音时消耗，用完可随时充值。",
        giftNote: "自备服务密钥的用户不消耗额度",
        sumCreditLabel: "可用额度",
        demoMutedLive: "🔇 正在面试录音——演示只显示回答、不播放声音，以免被麦克风带进通话。",
        kTarget: "STEP 1 · 你的目标", hTarget: "这一仗为谁而打",
        pTarget: "三项都可以跳过。会问的只有真正改变系统行为的字段——用不上的信息一个也不收集。",
        lblInterviewLanguage: "面试语言",
        capInterviewLanguage: "决定语音识别与 AI 回答的语言，下一次开始录音时生效。",
        phCompany: "目标公司（可跳过）",
        capCompany: "公司名会排在语音识别热词第一位，并成为备战与复盘的归档单位。",
        phRole: "岗位（可跳过）",
        capRole: "岗位会进入 AI 的语境，让回答更贴合这个职位。",
        sumTargetLabel: "面试目标",
        btnPickAmmo: "选择文件（简历 / 原稿）",
        resumeRecogFormat: "已读取简历「%@」——完成引导后，在备战工作台逐条确认解析出的弹药。",
        sumResumeLabel: "简历"
    )
    ...
    static let ja = OBStrings(
        ...
        k1: "STEP 2 / 資料の取り込み", h1step: "用意した答えを読み込む",
        ...
        k2: "STEP 3 / アクセス許可", h2step: "面接官の声を聞き取る",
        ...
        btnAllow: "アクセスを許可",
        k3: "STEP 5 / デモ", h3step: "聞いて、答える。試してみる",
        ...
        kKeys: "STEP 4 / 接続", hKeys: "コードを貼り付けるだけ",
        ...
        kGift: "STEP 4 / プレゼント", hGiftGranted: "60分ぶん、チャージ済み", hGiftBalance: "利用できる残り時間",
        ...
        kTarget: "STEP 1 / ターゲット", hTarget: "この面接は、どの会社のため？",
        pTarget: "3 つともスキップできます。実際に動作を変える項目だけを伺います——使わない情報は集めません。",
        lblInterviewLanguage: "面接の言語",
        capInterviewLanguage: "音声認識と AI の回答の言語を決めます。次の録音開始から有効です。",
        phCompany: "志望企業（スキップ可）",
        capCompany: "会社名は音声認識キーワードの最優先に入り、準備と振り返りの単位になります。",
        phRole: "職種（スキップ可）",
        capRole: "職種は AI の文脈に入り、回答がこのポジションに寄り添います。",
        sumTargetLabel: "面接ターゲット",
        btnPickAmmo: "ファイルを選択（履歴書 / 原稿）",
        resumeRecogFormat: "履歴書「%@」を読み込みました——完了後、ワークベンチで抽出結果を 1 件ずつ確認できます。",
        sumResumeLabel: "履歴書"
    )
}
